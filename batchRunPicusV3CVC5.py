...
for r1csFile in glob.glob('generated/O0/P*.r1cs', recursive=True): # PROD 
...
#for r1csFile in glob.glob('benchmarks/circomlib/*.r1cs', recursive=True): # DEV
    ...
    try:
        output = subprocess.run(["racket", "Picus/test-v3-uniqueness.rkt","--r1cs", r1csFile, "--weak", "--solver", "cvc5", "--timeout", "6000"], timeout=1800, capture_output=True) # prod
       ...
       # subprocess.run(["racket", "test-v3-uniqueness.rkt","--r1cs", r1csFile, "--weak", "--timeout", "3000"], timeout=600)
        if output.stdout.decode('utf-8').find('safety verified.') != -1:
            ...
            #put this in the map
            picusCircuitToStatus[rFileWithoutExtensionJustName] = { "tool" : "Picus v3/cvc5", "result" : "Not Verified"}
