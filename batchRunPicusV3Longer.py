...
#for r1csFile in glob.glob('benchmarks/circomlib/*.r1cs', recursive=True): # DEV
    ...
    try:
        output = subprocess.run(["racket", "Picus/test-v3-uniqueness.rkt","--r1cs", r1csFile, "--weak", "--timeout", "10000"], timeout=1000, capture_output=True) # prod
       ...
       # subprocess.run(["racket", "test-v3-uniqueness.rkt","--r1cs", r1csFile, "--weak", "--timeout", "3000"], timeout=600)
        if output.stdout.decode('utf-8').find('safety verified.') != -1:
            #put this in the map
            picusCircuitToStatus[rFileWithoutExtensionJustName] = { "tool" : "Picus v3/z3 Longer", "result" : "Weakly Verified"}
            ...
            #put this in the map
            picusCircuitToStatus[rFileWithoutExtensionJustName] = { "tool" : "Picus v3/z3 Longer", "result" : "Not Verified"}
            #markdownOutput+=('\n| '+ rFileWithoutExtensionJustName + ' | Picus | :x: |')
    
    except subprocess.TimeoutExpired:
         #print("Timeout!!!!!!!!!", rFileWithoutExtensionJustName)
         #put this in the map
         picusCircuitToStatus[rFileWithoutExtensionJustName] = { "tool" : "Picus v3/z3 Longer", "result" : "Timeout"}
